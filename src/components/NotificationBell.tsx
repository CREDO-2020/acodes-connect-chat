...
export function NotificationBell() {
  ...
  useEffect(()=>{ if(!user)return; const load=async()=>{const {count}=await supabase.from("notifications").select("id",{count:"exact",head:true}).eq("user_id",user.id).is("read_at",null);setCount(count??0)}; void load(); const ch=supabase.channel(`notifications-${user.id}`).on("postgres_changes",{event:"INSERT",schema:"public",table:"notifications",filter:`user_id=eq.${user.id}`},p=>{const row=p.new as {title?:string;body?:string|null};setCount(c=>c+1);toast.message(row.title??"New notification",{description:row.body??undefined})}).subscribe(); return()=>{void supabase.removeChannel(ch)} },[user]);
